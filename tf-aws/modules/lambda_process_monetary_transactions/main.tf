...
resource "aws_lambda_function" "process_mt_dlq" {
  function_name    = "process-mt-dlq-lambda"
  role             = var.process_monetary_transactions_lambda_role_arn
  handler          = "process-mt.handler"
  runtime          = "python3.12"
  filename         = "process-mt.zip" # Dummy file, the actual zip file will be uploaded in the lambda repo
  ...
  layers = [data.klayers_package_latest_version.psycopg.arn]
  tags = {
    Name = "process-mt-dlq-lambda"
  }
}
resource "aws_lambda_event_source_mapping" "dlq_trigger" {
  event_source_arn = var.mt_dlq_queue_arn
  function_name    = aws_lambda_function.process_mt_dlq.arn
  batch_size       = 1
  enabled          = true

  depends_on = [
    aws_lambda_permission.dlq_trigger
  ]
}

resource "aws_lambda_permission" "dlq_trigger" {
  statement_id  = "AllowExecutionFromDLQSQS"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.process_mt_dlq.function_name
  principal     = "sqs.amazonaws.com"
  source_arn    = var.mt_dlq_queue_arn
}